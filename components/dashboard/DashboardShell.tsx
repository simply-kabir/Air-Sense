...
export function DashboardShell() {
  ...
  const { station, loading, error, trendData, loadByCity, loadByCoords } = useCurrentStation('Delhi')
  ...
  const pageStyle = isKids ? {
    color: '#2D1B69', // ✅ FIX: Force global dark text for Kids Mode
    background: aqi <= 50
    ...
    color: 'var(--white)', // Normal mode keeps white text
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-4 py-5 md:px-8 md:py-7">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center gap-4 mb-6 animate-fade-up">
          <div className="flex-1">
            <h1 className="font-display font-700 text-2xl md:text-3xl mb-1 leading-tight" style={{ fontFamily: 'var(--font-display)' }}>
            ...
            {station && (
              <div className="flex items-center gap-1.5">
                ...
                <span className="text-xs font-mono" style={{ color: isKids ? '#7A5590' : 'var(--muted)' }}>
                  {station.name}
                  {isAutoDetect && userCoords && <span style={{ color: 'var(--cyan-400)' }}> · 📍 Your location</span>}
                </span>
              </div>
            )}
          </div>
          ...
          <div className="flex items-center gap-2 w-full sm:w-auto">
            <button
              ...
              style={isKids ? {
                ...
                border: isAutoDetect ? '1px solid rgba(0,212,255,0.4)' : '1px solid rgba(59,111,232,0.2)',
            ...
            <div className="flex-1 sm:w-72">
              {/* ✅ FIX: Added isKids={isKids} here! */}
              <CitySearch onSearch={handleSearch} loading={loading} isKids={isKids} placeholder="Search city…" />
            </div>
          </div>
        </div>
        ...
        {error && (
          <div className="mb-5 flex items-start gap-3 px-4 py-3 rounded-2xl bg-aqi-unhealthy/10 border border-aqi-unhealthy/25 animate-scale-in">
            ...
            <button
              ...
              className="text-xs font-mono text-aqi-unhealthy hover:underline flex items-center gap-1 shrink-0" // ✅ FIX: Removed hover:text-white
            >
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-5 mb-5">
          ...
          <div className="relative rounded-3xl overflow-hidden animate-fade-up-d2" style={{ background: isKids ? 'radial-gradient(ellipse at center,#E8F4FF 0%,#D0E8FF 100%)' : 'radial-gradient(ellipse at center,#0A1128 0%,#03070F 100%)', border: isKids ? '2px solid rgba(78,205,196,0.3)' : '1px solid rgba(59,111,232,0.15)', minHeight: 500 }}>
            <div className="absolute top-3 left-4 z-10 text-xs font-mono uppercase tracking-wider" style={{ color: isKids ? '#7A5590' : 'var(--muted2)' }}>
              {isKids ? '🌍 Your World' : 'Live Globe'}
            </div>
            {mapStations.length > 0 && (
              <div className="absolute top-3 right-4 z-10 px-2.5 py-1 rounded-full text-[10px] font-mono"
                // ✅ FIX: Made badge light for Kids Mode
                style={{ 
                  background: isKids ? 'rgba(255,255,255,0.8)' : 'rgba(6,11,26,0.7)', 
                  border: `1px solid ${isKids ? 'rgba(78,205,196,0.4)' : 'rgba(59,111,232,0.2)'}`, 
                  color: isKids ? '#2D1B69' : 'var(--muted)', 
                  backdropFilter: 'blur(8px)' 
                }}>
                {mapStations.length} stations nearby
              </div>
            )}
            <AQIGlobe userLat={globeLat} userLon={globeLon} stations={mapStations} isKids={isKids} style={{ position: 'absolute', inset: 0, width: '100%', height: '100%' }} />
          </div>
        </div>

        <div className="mb-5 animate-fade-up-d3"><AQITrendChart data={trendData} loading={loading} currentAqi={aqi} /></div>

        <div className="animate-fade-up-d4">
          <div className="mb-3">
            <span className="text-xs font-mono uppercase tracking-wider" style={{ color: isKids ? '#7A5590' : 'var(--muted)' }}>
        ...
        <div className="mt-5 animate-fade-up-d5"><PollutantStats station={station} loading={loading} isKids={isKids} /></div>

        {isKids && (
          <div className="mt-5 rounded-3xl p-5 animate-fade-up-d5" style={{ background: 'rgba(255,255,255,0.55)', border: '1px solid rgba(255,255,255,0.8)' }}>
            <h3 className="font-display font-700 text-sm mb-3" style={{ color: '#2D1B69', fontFamily: 'var(--font-display)' }}>🌍 Did You Know?</h3>
        ...
        <div className="mt-6 text-center animate-fade-in">
          <p className="text-xs font-mono" style={{ color: isKids ? 'rgba(45,27,105,0.35)' : 'var(--muted2)' }}>
            ...
            {cityAQIData && isAutoDetect && <span> · City avg from {cityAQIData.count} stations</span>}
          </p>
        </div>
      </div>
    </div>
